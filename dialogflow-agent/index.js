...
const media = {
	song: 'https://storage.googleapis.com/whodunnit-media/The_Black_Cat.mp3',
	siren: 'https://storage.googleapis.com/whodunnit-media/police_siren.mp3',
	door: 'https://storage.googleapis.com/whodunnit-media/door.mp3',
}

const actionHandlers = {

	[actions.QUIT](app, result, intent) {
		app.tell(responses.goodbye)
	},
	...
	[actions.NO_INPUT](app, result, intent) {
		...
		if(app.data.currentSuspect === undefined){
			...
			let hints = [0, 1].map(i => getHint(app, i))
				...
				.map(hint => voices.HAWKINS.speak(hint))
			app.data.hintIDX ++
	...
	[actions.UNKNOWN](app, result, intent) {
		ask(app, voices.HAWKINS.speak(voices.HAWKINS.speak(responses.help1)), hawkinsHelp)
	},
	...
	*/
	[actions.REPEAT](app, result, intent) {
		app.ask(app.data.prevInputPrompt, app.data.prevNoInputPrompts)
	},
	...
	*/
	...
	[actions.INTRO_INTERROGATE_SUSPECTS](app, result, intent) {
		// console.log(actions.INTRO_INTERROGATE_SUSPECTS, intent)
		ask(app, voices.HAWKINS.speak(responses.suspects.list + responses.connective.selectsuspect))
	},
	...
	*/
	[actions.QUESTION_SUSPECT](app, result, intent) {
		...
		console.log(intent, intents.OTHERSUSPECTALIBI)
		switch(intent) {
		...
		if(realizations.acceptedIntents.variations[v][suspect].indexOf(intent) < 0) {
		...
		if(
			...
			realizations.acceptedIntents.variations[v][suspect].indexOf(intent) > -1 // some responses have too much info, or don't fit with the realziation hints
		) {  
		...
		// only serve hint on screens
		if(app.hasSurfaceCapability(app.SurfaceCapabilities.SCREEN_OUTPUT)){
			let cheerio = require('cheerio')
			let hint = getHint(app)
			app.data.hintIDX ++
			// strip xml from hint
			let suggestion = cheerio.load(hint.text).text()
			response.addBasicCard(app.buildBasicCard(suggestion)).setTitle('Hint')
		}
...
*/
exports.devhttp = exports.http
exports.stagehttp = exports.http